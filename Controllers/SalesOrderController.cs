...
namespace SalesOrder.Controllers
{
    ...
    public class SalesOrderController(IOrderRepository orderRepository) : Controller
    {
        ...
        public async Task<IActionResult> Index(string keyword, DateTime? dateFrom, DateTime? dateTo, int pageNumber = 1)
        {
            int pageSize = 5;
            var query = _orderRepository.GetOrdersQueryable(keyword, dateFrom, dateTo);
